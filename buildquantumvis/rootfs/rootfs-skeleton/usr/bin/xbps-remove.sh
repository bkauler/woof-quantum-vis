...
if [ "$SUMii1" != "$SUMii2" ];then
 ...
 cat - <<<${PKGSii2} >/tmp/woofQV/PKGSii2
 for aNVR in ${PKGSii1}
 do
  [ -z "$aNVR" ] && continue
  grep -q -x -F -f /tmp/woofQV/PKGSii2 <<<${aNVR}
  if [ $? -ne 0 ];then
   grep -v -G "^${aNVR}|" /root/.packages/user-installed-packages > /tmp/woofQV/new-uip
   mv -f /tmp/woofQV/new-uip /root/.packages/user-installed-packages
