...
if [ ! -f /usr/bin/X ];then
 ...
 if [ -e /.brokenvideo ];then #20211014 file created in init (in initrd).
  #rm -f /.brokenvideo #remove in xorgwizard-cli
  rm -f /.XLOADED 2>/dev/null #20220801
  rm -f /var/local/quicksetup-test-success 2>/dev/null
  xorgwizard-cli
  rm -f /.brokenvideo #just in case didn't get deleted.
 fi
 if [ -s /var/local/quicksetup-test-success ];then #140209 /usr/sbin/quicksetup writes to this file.
  rm -f /.XLOADED #140209 avoid clash with this mechanism, see /usr/bin/xwin  20220801
  xorgwizard-cli
 fi
